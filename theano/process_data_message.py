...
def get_extra_feature(data):
    extra_fea = data['extra_feature'].split(',')
    comment = log(1+data['comment'])
    forword = log(1+data['forword'])
    like = log(1+data['like'])
    extra_fea.extend([comment, forword, like])
    return ' '.join([str(val) for val in extra_fea])
...
def load_json_file(filename, extra_fea_file, vocab, label, fold_map):
    ...
    with open(filename) as f:
        ...
        for line in f:
            ...
            content = d['content']#.encode("utf8")
            tmp_word_list = []
            for wordtag in d['words']:
                index = wordtag.rfind('/')
                word, tag = wordtag[0:index], wordtag[index+1:]
                tmp_word_list.append(word)
            ori_rev = ' '.join(tmp_word_list)
            mid = d['id'].encode('utf8', 'ignore')

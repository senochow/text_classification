...
def build_data_train_test(data_folder, clean_string=True):
    """
    Loads data, 0 for train, 1 for test
    """
    revs = []
    ...
    with open(train_file, "rb") as f:
        for line in f:
            uid, text, label = line.rstrip().split('\t')
            rev = []
            rev.append(text)
            if clean_string:
            ...
            datum  = {"y":int(label),
                      ...
                      "uid": uid,
                      "split": 0} # 0 for train, 1 for test
            revs.append(datum)
...
if __name__=="__main__":
    ...
    data_folder = [pos_file, neg_file]
    print "loading data...",
    revs, vocab = build_data_cv(data_folder, 5, clean_string=True)
